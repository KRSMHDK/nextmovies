  ...
  return (
    <div className="h-auto max-w-screen-xl pt-5 pl-5 mx-auto mt-2 ">
      ...
      <ul className="flex mt-3 overflow-x-scroll rounded-xl">
        {tvDetails.images.backdrops.length === 0 ? (
          <div>No Backdrops Found </div>
        ) : (
          tvDetails.images.backdrops.map((tv) => (
            <li key={tv.file_path} className="flex-shrink-0 ">
              <Image
                className="mx-auto "
                placeholder="blur"
                ...
                width={533}
                height={300}
                src={`https://www.themoviedb.org/t/p/w533_and_h300_bestv2${tv.file_path}`}
              />
            </li>
          ))
        )}
